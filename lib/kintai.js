...
var StatsReport = function (_Report) {
    _inherits(StatsReport, _Report);

    function StatsReport() {
        _classCallCheck(this, StatsReport);

        return _possibleConstructorReturn(this, (StatsReport.__proto__ || Object.getPrototypeOf(StatsReport)).apply(this, arguments));
    }
    ...
    _createClass(StatsReport, [{
        ...
        value: function collectReport(page, done) {
            var result = page.evaluate(function () {
                ...
                var stats_table = document.querySelectorAll('table')[13];
                var cells = stats_table.querySelectorAll('td');
                var report = {};
                var key = void 0;
                ...
                var collectMonthly = function collectMonthly() {
                    var days_table = document.querySelector('#DUM_EZZOPCK-0001').parentNode.parentNode.parentNode.parentNode;
                    ...
                    Array.prototype.forEach.call(days_table.children, function (day) {
                        ...
                        result.push([date, up1, down1, up2, down2].map(function (v) {
    ...
    _createClass(KintaiServer, [{
        ...
        value: function run() {
            ...
            server.listen(this.port, function (request, response) {
                ...
                var controllers = {
                    '/': function _() {
                        _this11.performRoot(response);
                    },
                    '/up': function up() {
                        _this11.authorize(parameters, response, function () {
                            return _this11.performUp(response);
                        });
                    },
                    '/down': function down() {
                        _this11.authorize(parameters, response, function () {
                            return _this11.performDown(response);
                        });
                    },
                    '/report.json': function reportJson() {
                        _this11.authorize(parameters, response, function () {
                            return _this11.performStatsReport(response);
                        });
                    },
                    '/report.txt': function reportTxt() {
                        _this11.authorize(parameters, response, function () {
                            return _this11.performReport(response);
                        });
                    }
                };
        ...
        value: function performKintai(job, response) {
            ...
            if (this.setupCompleted()) {
                job.run(function (content) {
                    _this12.returnsText(content, response);
                });
            } else {
                response.statusCode = 400;
                response.setHeader('Content-Type', 'text/plain; charset=utf-8');
        ...
        key: 'performStatsReport',
        value: function performStatsReport(response) {
            var report = new StatsReport(this.kintai_url, this.staff_id, this.password);
            this.performKintai(report, response);
        }
    }, {
        key: 'perform404',
