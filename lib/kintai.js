...
var KintaiServer = function () {
    function KintaiServer(KINTAI_URL, STAFF_ID, PASSWORD, PORT) {
        ...
        value: function run() {
            ...
            server.listen(this.port, function (request, response) {
                var controllers = {
                    ...
                    '/up': function up() {
                        _this10.performUp(response);
                    },
                    '/down': function down() {
                        _this10.performDown(response);
                    },
                    '/report': function report() {
                        _this10.performReport(response);
                    }
                };

                var controller = controllers[request.url];
                if (!controller) {
...
var server = new KintaiServer(system.env['KINTAI_URL'], system.env['STAFF_ID'], system.env['PASSWORD'], system.env['PORT']);
server.run();
